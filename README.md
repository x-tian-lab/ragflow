...
## 当前状态：MVP 验收通过（spec v0.2，eval v3，2026-07-13）

- 全语料 59,097 chunks，A 级引用占比 94%，C 级为零
- BM25 检索基线：file_hit@5 = 94.5%，条号级 loc_hit@5 = 92.7%，单查 0.2-0.3s
- 端到端（BM25 + DeepSeek-chat，60 题）：自动判分 76.7%、拒答 5/5、延迟 60/60 <5s、引用命中 55/55；14 道判负题 100% 人工复核后实际准确率 **≈93.3%**——对照验收线（>80% 准确率 / >90% 引用命中 / <5s）**全部通过**（人工复核口径，per spec §8）
- 自动分与实际质量的 16.6 个百分点差距全部来自字符串判分器假阴性 → 下一阶段第一优先级是 LLM-judge
- 遗留真实缺陷：表格行级检索粒度（M05/X03）、总则/具体条款消歧（M02）、多依据穷举（T09）——详见 spec_dev.md 验收记录与 eval_full_report.md
